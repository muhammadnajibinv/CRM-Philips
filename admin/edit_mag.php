...
<body>
    ...
    <div class="container">
        ...
        <div class="row">
            <div class="col-lg-12">
                <h1 class="page-header" align="center">Mag</h1>
            </div>
		</div>
		
<?php
$query=mysql_query("SELECT * FROM tbl_mag WHERE id_mag='".$_GET['id_mag']."'");
while($hasil=mysql_fetch_array($query))
{
$id_mag=$hasil['id_mag']; //isi tabel data
$nama_mag=$hasil['nama_mag']; //isi tabel data
}
?>
...
<div class="row">
<div class="col-md-12">
            <form action="proses_pencarian_mag.php" method="post" >
	...
	<form method="post" action="proses_edit_mag.php">
        <div class="col-sm-6">
            <h4>Edit Mag :</h4>
            <div class="panel panel-default">
                <div class="panel-body form-horizontal payment-form">
                    <div class="form-group">
                        <label for="nama_mag" class="col-sm-3 control-label">Nama Mag</label>
                        <div class="col-sm-9">
                            <input type="text" class="form-control" id="nama_mag" name="nama_mag" value="<? echo"$nama_mag"; ?>" maxlength="50" required autofocus>
                        </div>
						<input type="hidden" class="form-control" id="id_mag" name="id_mag" value=<?php echo"$id_mag";?>>
                    </div>
                    <div class="form-group">
...
$query=mysql_query("SELECT * FROM tbl_mag order by id_mag desc limit $posisi,$batas");
While($hasil=mysql_fetch_array($query))
{
$id_mag=$hasil['id_mag'];
...
<td> <? echo"$id_mag"; ?> </td>
<td> <? echo"$nama_mag"; ?> </td>
<td><p data-placement="top" data-toggle="tooltip" title="Edit"><a class="btn btn-primary btn-xs" href="edit_mag.php?id_mag=<? echo $id_mag;?>"><span class="glyphicon glyphicon-pencil"></span></button></p></td>
<td><p data-placement="top" data-toggle="tooltip" title="Delete"><a href="javascript:if(confirm('Anda Yakin Akan Menghapus Mag <? echo $nama_mag ?> Ini ?')){document.location='delete_mag.php?id_mag=<?php echo $id_mag; ?>';}">
...
<?}?> 
 ...
 <?
$query2=mysql_query("select * from tbl_mag");
...
if (1 != $paging){//tanda != berarti perintah akan dijalankan jika $paging tidak sama dengan 1, copyright all teknik paging by go_blind_hacker, powered by V-boys_studio
	$back=$paging-1;
	echo "<li><a href='mag.php?paging=$back'><span class='glyphicon glyphicon-chevron-left'></span></a></li>";
	}
	else{
...
if ($paging != $jumlah_halaman){    
	$next=$paging+1;
	echo "<li><a href='mag.php?paging=$next'><span class='glyphicon glyphicon-chevron-right'></span></a></li>";
	}
	else
	{
