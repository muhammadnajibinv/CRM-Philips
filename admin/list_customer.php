...
$query=mysql_query("SELECT * FROM tbl_customer order by id_customer desc limit $posisi,$batas");
...
<form method="post" action="proses_blokir_customer.php?id_customer=<?php echo $id_customer; ?>">
...
<form method="post" action="proses_ganti_akses_customer.php?id_customer=<?php echo $id_customer; ?>">
...
<?}?>
 ...
 <?
$query2=mysql_query("select * from tbl_customer");
...
if (1 != $paging){//tanda != berarti perintah akan dijalankan jika $paging tidak sama dengan 1, copyright all teknik paging by go_blind_hacker, powered by V-boys_studio
	$back=$paging-1;
	echo "<li><a href='list_customer.php?paging=$back'><span class='glyphicon glyphicon-chevron-left'></span></a></li>";
	}
	else{
...
if ($paging != $jumlah_halaman){    
	$next=$paging+1;
	echo "<li><a href='list_customer.php?paging=$next'><span class='glyphicon glyphicon-chevron-right'></span></a></li>";
	}
	else
	{
